...
- 텍스처 1개당 2칸 사용
  - texture
  - sampler
- storage buffer는 텍스처 뒤 슬롯부터 1칸씩 사용
...
@group(1) @binding(5) var<storage, read> glyphs: array<Glyph>;
```
...
`storages`는 두 방식 모두 지원합니다.

- 권장 객체형: `storages: { glyphs: { struct: { pos: "vec2f", size: "vec2f" } } }`
- 직접 타입 지정: `storages: { glyphs: { type: "array<Glyph>" } }`
- 이전 배열형도 계속 지원: `[{ name: "glyphs", type: "array<Glyph>" }]`

`access`를 생략하면 기본값은 `read`이고, binding은 직접 적지 않아도 됩니다.

### pass에서 storage 연결하기

`buildShader()`로 선언한 storage는 이름 기준으로 연결하는 쪽이 가장 편합니다.

`pass.createStorage(name, options)`는 이제 `renderer.addPass(pass)` 전에 호출해도 됩니다.
이 경우 실제 GPU 버퍼 생성은 pass 초기화 시점에 자동으로 이어집니다.

```js
const glyphs = pass.createStorage("glyphs", {
  length: 256,
  initialValues: [
    { pos: [0, 0], size: [16, 16] },
  ],
...
이미 만든 `StorageBlock`이나 GPUBuffer를 연결할 수도 있습니다.

```js
pass.setStorage("glyphs", glyphBlock);
...
`pass.createStorage(name, options)`는 shader contract를 보고 아래를 자동으로 맞춥니다.

- binding
- buffer type (`read-only-storage` / `storage`)
- struct definition
- 그리고 기본적으로 해당 이름에 바로 연결합니다. (`bind: false`로 비활성화 가능)

가변 길이 storage(`array<T>` 또는 마지막에 unsized array가 있는 struct)는 `length`, `byteLength`, `initialData`, `arrayBuffer` 중 하나가 필요합니다.
